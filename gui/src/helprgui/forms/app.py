...
class HelprAppForm(forms.AppForm):
    ...
    def analysis_finished_callback(self, state_obj: type(ModelBase), results: dict):
        ...
        if status == -1:
            ...
            state_obj.ensemble_data = results.get('ensemble_data')

            state_obj.design_curve_plot = results.get('design_curve_plot')
            state_obj.design_curve_data = results.get('design_curve_data')

            state_obj.fad_plot = results.get('fad_plot')
            state_obj.fad_data = results.get('fad_data')

            state_obj.cycle_plot = results.get('cycle_plot')
            state_obj.cycle_data = results.get('cycle_data')
            state_obj.cycle_cbv_plots = results.get('cycle_cbv_plots')

            state_obj.pdf_plot = results.get('pdf_plot')
            state_obj.pdf_data = results.get('pdf_data')

            state_obj.cdf_plot = results.get('cdf_plot')
            state_obj.cdf_data = results.get('cdf_data')
