 ...
 * Tests file path display, label, and value changes.
 *
 * Run with: python run_qml_tests.py --test test_file_selector.qml
 */
import QtQuick
...
ApplicationWindow {
    ...
    function test_01_creation_with_path() {
        log("Test: FileSelector creation with path");
        tester.create_file_path_field();
        assertEqual(tester.get_string_value(), "/path/to/data.csv", "Initial path value");
    }

    function test_02_label_property() {
        log("Test: FileSelector label property");
        tester.create_file_path_field();
        assertEqual(tester.get_string_label(), "Input File", "Label is 'Input File'");
    }

    function test_03_value_property() {
        log("Test: FileSelector value property");
        tester.create_file_path_field();
        let value = tester.get_string_value();
        assert(value.length > 0, "Value is not empty");
        assert(value.indexOf(".csv") > 0, "Value contains .csv extension");
    }

    function test_04_empty_path_creation() {
        log("Test: FileSelector empty path creation");
        tester.create_empty_file_path_field();
        assertEqual(tester.get_string_value(), "", "Empty path value");
        assertEqual(tester.get_string_label(), "Input File", "Label is 'Input File'");
    }

    function test_05_set_value() {
        log("Test: FileSelector set value");
        tester.create_file_path_field();
        tester.set_string_value("/new/path/to/file.csv");
        assertEqual(tester.get_string_value(), "/new/path/to/file.csv", "Value updated");
    }

    function test_06_clear_value() {
        log("Test: FileSelector clear value");
        tester.create_file_path_field();
    ...
    function test_07_multiple_field_creation() {
        log("Test: Multiple FileSelector creation");
        tester.create_file_path_field();
        assertEqual(tester.get_string_value(), "/path/to/data.csv", "First: with path");

        tester.create_empty_file_path_field();
        assertEqual(tester.get_string_value(), "", "Second: empty");

        tester.create_file_path_field();
        assertEqual(tester.get_string_value(), "/path/to/data.csv", "Third: back to path");
    }
}
