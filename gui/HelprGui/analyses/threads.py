...
from state.models import State, do_analysis
...
class AnalysisThread(threading.Thread):
    ...
    state : State
        State model representing current form parameters.
    ...
    # list of analyses waiting to execute. Each entry is tuple: (id, state_copy, started_callback, finished_callback)
    _queue: list = []
    ...
    def __init__(self, state: State, num_processes=None):
        """Initializes pool with specified process count.

        Parameters
        ----------
        state : State
            state model object representing parameters in form.
    ...
    def run(self):
        ...
        while True:
            if self._queue and not self._do_shutdown:  # process one at a time
                ...
                (analysis_id, state_copy, started_callback, finished_callback) = self._queue.pop(0)

                # get new analysis state object, store record of params in it, and start analysis via dict of data.
                self._num_active += 1
                ...
                if settings.USE_SUBPROCESS:
                    ...
                    if settings.VERBOSE:
                        log.info(f"NEW ANALYSIS {analysis_id} with params:")
                        log.info(pprint.pformat(params, indent=20, sort_dicts=True))
                        log.info("END of params\n")

                    future = self.pool.submit(do_analysis, analysis_id, params, self.status_dict)
                    ...
                    try:
                        res = do_analysis(analysis_id=analysis_id, params=params, global_status_dict=self.status_dict)
                        self._process_results(future=None, results=res)
                    except Exception as err:
                        log.exception("Exception occurred during analysis call")
    ...
    def request_new_analysis(self, started_callback: callable, finished_callback: callable) -> int or None:
        ...
        """
        # if self._num_active >= self._max_active:
        #     self._log(f'ERROR: cannot start analysis - too many active ({self._num_active} / {self._max_active})')
        #     return None
        ...
        state_data = copy.deepcopy(self.state)
        self._queue.append((analysis_id, state_data, started_callback, finished_callback))
        return analysis_id
    ...
    def _process_results(self, future, results=None):
        ...
        self._log("Begin results processing.")
        self._log(f"Process results for analysis {analysis_id}")
        self._num_active -= 1
