...
def get_unit_type_from_unit_key(key: str):
    ...
    """
    unit_types = [Distance, SmallDistance, Pressure, Temperature, Fracture, Percent]
    key = key.lower().strip()
...
class UnitType:
    ...
    unit_data : dict
        {string: float or function} pairs indicating the unit and conversion factor or function that converts from the standard unit
...
class SmallDistance(UnitType):
    ...
    """
    label = 'dist_sm'
    unit_data = {'m': 1, 'mm': 0.001, 'in': 0.0254}
    display_units = ['m', 'mm', 'in']
    m = 'm'
    mm = 'mm'
    inch = 'in'
    std_unit = 'm'
